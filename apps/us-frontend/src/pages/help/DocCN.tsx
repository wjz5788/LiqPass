...
const CONTENT = `爆仓保 / LiqPass — 基金申请用帮助文档（中文最终版）
...
一、为什么选择这些交易所
我们优先对接流动性与成交量排名靠前的中心化合约平台（Binance、OKX、Bybit 等），因为这些平台的市场深度高、爆仓标识字段标准化、接口稳定性强，有助于验证流程的标准化与结果的可复算性。
...
二、为什么用户愿意绑定交易所信息
核心目的
绑定的唯一目的，是为了验证爆仓事件的真实性。当用户提交爆仓订单号时，系统通过只读接口核查：
订单是否真实存在；
时间、标的、杠杆、结算价等是否匹配；
...
安全与最小授权
一个自然人用户通常会拥有多个交易所账户，但在使用爆仓保时，只需绑定其中一家交易所即可；
授权类型为只读权限（Read-Only），无交易、提币、转账能力；
我们建议将主账户与高杠杆账户分离，以保障资金安全；
举例说明：
...
三、产品与理赔流程
当前产品
目前仅提供基于本金与杠杆倍数的定额爆仓保障。用户在购买时输入本金与杠杆，系统自动计算保费与对应赔付额。
...
理赔流程（被动机制）
爆仓保为被动理赔机制，不会主动检测用户交易。用户在爆仓后提交订单号或 JSON 文件，系统在本地核对订单详情与保单，信息匹配则发起赔付；若自动核对失败，用户可提交截图与说明，由人工复核。
...
五、定价与风控说明
定价原则
保费由系统基于风险概率、赔付上限与运营成本的综合模型计算，确保资金池长期可持续、公平透明。
...
Base 主网合约：0xC423C34b57730Ba87FB74b99180663913a345D68
...
export default function DocCN() {
  return (
    <article className="mx-auto max-w-4xl rounded-2xl border bg-white/80 p-6 shadow-sm">
      <h2 className="text-2xl font-bold mb-4">中文文档</h2>
